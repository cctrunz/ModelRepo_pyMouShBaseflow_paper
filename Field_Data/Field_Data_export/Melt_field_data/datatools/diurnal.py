...
class DiurnalExtrema(object):
        ...
        for day in self.timeseries.index.to_period('D').unique():
            ...
            if self.predictTiming:
                ...
                if self.minfirst and self.maxnextday:
                    ...
                    maxVal, maxTime = self.get_good_extrema(self.timeseries,
                                                            (minTime, minTime
    ...
    def predict_extrema(self):
        ...
        for day in calib_data.index.to_period('D').unique():
            ...
            minVal, minTime = self.get_good_extrema(calib_data, day, 'min')
            if minTime and self.minfirst and self.maxnextday:
                # find diurnal maximum in 18 hour window after minimum
                maxVal, maxTime = self.get_good_extrema(calib_data,
                                                        (minTime, minTime
                                                         + pd.Timedelta(hours=18)),
                                                        'max')
            else:
                maxVal, maxTime = self.get_good_extrema(calib_data, day, 'max')
    ...
    def get_good_extrema(self, ts, window, min_or_max):
        '''
    ...
    def plot(self, generate_figure=False, *args, **kwargs):
        if not self.generate_figure or not generate_figure:
            exit()
        fig = plt.figure()
        plt.plot(self.timeseries, linewidth=1)
        # plt.plot(timeseries,'.c',markersize=1)
        plt.plot(self.df.min_time, self.df.min_val, '.')
        plt.plot(self.df.max_time, self.df.max_val, '.r')
        plt.ylabel(self.timeseries.name)
...
def on_boundary(timeseries, window, extrema_tuple, val_type) -> bool:
    """ 
