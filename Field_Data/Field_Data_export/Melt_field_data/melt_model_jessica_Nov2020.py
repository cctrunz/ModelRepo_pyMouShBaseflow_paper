...
# read in data from GC-NET weather stations
...
class WeatherStation:
    ...
    def calc_albedo(self, daily=True):
        ...
        if self.solar is not None and self.reflected is not None:
            incoming = self.solar_corrected if hasattr(
                self, 'solar_corrected') else self.solar
            incoming = c_rolling(incoming, '2H')
            outgoing = c_rolling(self.reflected, '2H')

            self.albedo = outgoing / incoming

            if daily:
                incoming_max = incoming.resample('D').max()
                idx = pd.DatetimeIndex(
                    incoming.resample('D').apply(lambda x: x.idxmax()).values)
                outgoing_max = outgoing[idx].resample('D').apply(lambda x: x)

                self.albedo = outgoing_max / incoming_max
    ...
    def calc_melt(self,
        ...
        df['incoming'] = c_rolling(self.solar_corrected, '2H') if hasattr(
            self, 'solar_corrected') else c_rolling(self.solar, '2H')
...
def melt_equ(temperature,
    ...
    Returns:
        melt_rate: mm melt water equivelent per timestep
    """
