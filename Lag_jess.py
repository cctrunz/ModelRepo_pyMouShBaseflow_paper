...
meltwater_input = Qin_real(time, meltmodel.Q.to_numpy(),  meltmodel.soy.to_numpy())
...
Q = np.array(bf1_fix.dict['meltwater_input_moulin'])
h = np.array(bf1_fix.dict['head'])
t = np.array(bf1_fix.dict['time'])
#%%
plt.figure()
plt.plot(t/3600,Q*2000, color='grey', label='Qin')
plt.plot(h_soy/3600,h_real, color='black', label='Measured head')
plt.plot(t/3600,h, color='red', label='Modeled head')
plt.legend()
...
Q = np.array(cylinder10.dict['meltwater_input_moulin'])
h = np.array(cylinder10.dict['head'])
t = np.array(cylinder10.dict['time'])
#%%
plt.figure()
plt.plot(t/3600,Q*2000, color='grey', label='Qin')
plt.plot(h_soy/3600,h_real, color='black', label='Measured head')
